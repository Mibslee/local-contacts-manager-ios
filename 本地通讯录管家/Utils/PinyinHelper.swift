...
class PinyinHelper {
    ...
    // 使用嵌套 enum 实现 lazy 初始化，首次访问时才加载 500+ 条目
    private enum PinyinData {
        static let map: [String: String] = [
            ...
            "羊舌": "yangshe", "微生": "weisheng", "梁丘": "liangqiu", "左丘": "zuoqiu",
            "东门": "dongmen", "西门": "ximen", "南宫": "nangong"
        ]
    }

    private static var pinyinMap: [String: String] { PinyinData.map }
    ...
    static func toPinyin(_ chinese: String) -> String {
        ...
        while i < chars.count {
            ...
            if i + 1 < chars.count {
                let twoChars = String(chars[i...i+1])
                if let pinyin = pinyinMap[twoChars] {
                    result += pinyin.capitalized
                    i += 2
                    continue
                }
            }

            if let pinyin = pinyinMap[char] {
                result += pinyin.capitalized
            } else {
                result += char
            }
    ...
    static func toPinyinInitials(_ chinese: String) -> String {
        var result = ""
        let chars = Array(chinese)
        ...
        while i < chars.count {
            ...
            if i + 1 < chars.count {
                let twoChars = String(chars[i...i+1])
                if let pinyin = pinyinMap[twoChars] {
                    result += String(pinyin.prefix(1)).uppercased()
                    i += 2
                    continue
                }
            }
